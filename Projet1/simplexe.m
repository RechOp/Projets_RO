...
s(find(s == 0)) = 1; % s(i)== 1 si b(i) >=0 , s(i)== -1 si b(i) < 01
A = diag(s)*A;
b = b.*s;
%
%% PHASE I
...
%
z = c(JB)'*GAMMA(:,J);
pentes = c' - z; % pentes contient les mesures des pentes des chemins partant du sommet courant
...
while any(OPT*pentes > 0) % Y-a-t-il un chemin améliorant ?
    ...
    indFind = indJNP;
    for i=1:m 
        indFind = find(GAMMA(i,indFind) <= 0);    %JNP : indices j des vecteurs Aj définissant des chemins améliorant (xj > 0)
    end
    JNP = indFind;
    ...
    [~,indalpha] = max(c(indJNP) - sum(GAMMA(JN)*c(JN))); %indalpha : indice dans pente du chemin améliorant de plus forte pente
    ...
    indVEC = find(VEC > 0);
    [~,indbeta] = min(GAMMA(indVEC,n+1)./VEC(indVEC));
    % beta = JB(indbeta)
    %
    ...
    GAMMAMOINS = GAMMA(:,alpha)*GAMMA(indbeta,:);
    GAMMAMOINS = GAMMAMOINS/GAMMA(indbeta,alpha);
    GAMMAMOINS(indbeta,:)=zeros(1,n+1);
    GAMMAMOINS(:,alpha)=zeros(m,1);
