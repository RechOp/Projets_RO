...
%
z = c*GAMMA(:,JB);
pentes = c - z(JB); % pentes contient les mesures des pentes des chemins partant du sommet courant
...
while any(OPT*pentes > 0) % Y-a-t-il un chemin améliorant ?
    ...
    JNP = find(GAMMA(:,indJNP) > 0);    %JNP : indices j des vecteurs Aj définissant des chemins améliorant (xj > 0)
    ...
    indVEC = find(VEC > 0);
    ind = setdiff(JN,indVEC);
    [~,indbeta] = min(GAMMA(:,ind)./VEC(ind));
    ...
    GAMMAMOINS = GAMMA(JN,alpha)*GAMMA(indbeta,JN);
